...
import catalogLibraryIndex from '../dist/catalogs/index.json' assert { type: 'json' };
...
async function main() {
  ...
  const indexFileUri = pathToFileURL(`./dist/catalogs/${indexDefinitionFileName}`).toString();
  ...
  const schemaPromises = Object.entries(indexDefinitionContent.schemas).map(async ([name, schema]) => {
    ...
    const schemaFile = resolve(`./dist/catalogs/${baseFolder}/${schema.file}`);
